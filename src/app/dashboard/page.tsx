...
import { BookOpen, CheckCircle, ArrowRight, Award, Edit3, RefreshCw, User, LayoutGrid, MessageSquare, GraduationCap, ArrowLeft } from 'lucide-react';
...
export default function EnglishLearningDashboard() {
  ...
  const [currentView, setCurrentView] = useState<'hub' | 'lessons'>('hub'); // 'hub' ያማሩትን ካርዶች ያሳያል፣ 'lessons' ትምህርቱን ያሳያል
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        {/* እይታ 1፦ ዋናው ማውጫ (Hub View with the beautiful 4 cards) */}
        {currentView === 'hub' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* ካርድ 01 - Syllabus Modules (ይህንን ሲነካ ወደ ትምህርቱ ይወስደዋል) */}
              <div className="bg-slate-800/50 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl">
                <div>
                  <span className="w-8 h-8 rounded-lg bg-indigo-600/20 text-indigo-400 font-bold flex items-center justify-center text-sm mb-4 border border-indigo-500/20">
                    01
                  </span>
                  <h3 className="text-lg font-bold text-white mb-2">Syllabus Modules</h3>
                  <p className="text-sm text-slate-400 leading-relaxed font-light">
                    በሙያዎ እና በዕለት ተዕለት ሕይወትዎ የሚጠቅሙትን የእንግሊዘኛ ትምህርቶች በይነተገናኝ በሆነ መልኩ ይማሩ::
                  </p>
                </div>
                <button 
                  onClick={() => setCurrentView('lessons')}
                  className="mt-6 w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm transition-all text-center block shadow-lg shadow-indigo-950/50"
                >
                  ትምህርቱን ጀምር
                </button>
              </div>

              {/* ካርድ 02 - Vocabulary Builder */}
              <div className="bg-slate-800/50 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl">
                <div>
                  <span className="w-8 h-8 rounded-lg bg-fuchsia-600/20 text-fuchsia-400 font-bold flex items-center justify-center text-sm mb-4 border border-fuchsia-500/20">
                    02
                  </span>
                  <h3 className="text-lg font-bold text-white mb-2">Vocabulary Builder</h3>
                  <p className="text-sm text-slate-400 leading-relaxed font-light">
                    ቃላትን በፍጥነት የሚያጠኑበት እና አጠቃቀማቸውን የሚለማመዱበት ልዩ ክፍል::
                  </p>
                </div>
                <button 
                  onClick={() => setCurrentView('lessons')}
                  className="mt-6 w-full py-3 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-semibold text-sm transition-all text-center block shadow-lg shadow-fuchsia-950/50"
                >
                  ትምህርቱን ጀምር
                </button>
              </div>

              {/* ካርድ 03 - AI Chat Tutor */}
              <div className="bg-slate-800/50 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl">
                <div>
                  <span className="w-8 h-8 rounded-lg bg-emerald-600/20 text-emerald-400 font-bold flex items-center justify-center text-sm mb-4 border border-emerald-500/20">
                    03
                  </span>
                  <h3 className="text-lg font-bold text-white mb-2">AI Chat Tutor</h3>
                  <p className="text-sm text-slate-400 leading-relaxed font-light">
                    ከአርቲፊሻል ኢንተለጀንስ አስተማሪ ጋር በቀጥታ በድምፅ እና በፅሁፍ ይለማመዱ::
                  </p>
                </div>
                <button 
                  onClick={() => setCurrentView('lessons')}
                  className="mt-6 w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-sm transition-all text-center block shadow-lg shadow-emerald-950/50"
                >
                  ትምህርቱን ጀምር
                </button>
              </div>

              {/* ካርድ 04 - Practical English Hub */}
              <div className="bg-slate-800/50 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl">
                <div>
                  <span className="w-8 h-8 rounded-lg bg-blue-600/20 text-blue-400 font-bold flex items-center justify-center text-sm mb-4 border border-blue-500/20">
                    04
                  </span>
                  <h3 className="text-lg font-bold text-white mb-2">Practical English Hub</h3>
                  <p className="text-sm text-slate-400 leading-relaxed font-light">
                    ማንበብ፣ የዕለት ተዕለት ውይይቶችን መለማመድ እና የፅሁፍ ብቃትዎን በአንድ ቦታ ያሳድጉ::
                  </p>
                </div>
                <button 
                  onClick={() => setCurrentView('lessons')}
                  className="mt-6 w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-sm transition-all text-center block shadow-lg shadow-blue-950/50"
                >
                  ትምህርቱን ጀምር
                </button>
              </div>
        ...
        {/* እይታ 2፦ የትምህርት ክፍሉ (Lessons Player View - fully functional and responsive) */}
        {currentView === 'lessons' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <button 
              ...
              className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white bg-slate-800/60 px-3 py-2 rounded-xl border border-slate-800 transition-all"
            >
              <ArrowLeft className="w-4 h-4" /> ወደ ዋናው ማውጫ ተመለስ
            </button>
