          ...
          <div ref={tabContainerRef} className="flex items-center gap-1.5 overflow-x-auto pb-2 px-2 scrollbar-none snap-x">
            {(["overview", "grammar", "vocabulary", "reading", "speaking", "quiz"] as const).map((tab) => {
              ...
              return (
                ...
                >
                  {tab === "overview" ? "📑 Overview" : tab === "grammar" ? "📝 Grammar" : tab === "vocabulary" ? "🧠 Words" : tab === "reading" ? `📖 Reading (${readingStories.length})` : tab === "speaking" ? "🗣️ AI Speak" : `🎯 Quiz (${quizQuestions.length})`}
                </button>
              );
            })}
          </div>
            ...
            <div className="bg-[#121b2e] p-5 rounded-2xl border border-slate-800 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                ...
                <span className="text-[10px] bg-purple-900/50 text-purple-300 font-mono px-2 py-0.5 rounded border border-purple-700/30">
                  {!quizFinished ? `Question ${currentQuestionIndex + 1} of ${quizQuestions.length}` : "Finished"}
                </span>
              </div>
              ...
              {!quizFinished ? (
                <>
                  ...
                  <div className="pt-2">
                    ...
                    {quizQuestions[currentQuestionIndex].amharicHint && (
                      <span className="text-[11px] text-slate-500 font-light block mt-1 bg-slate-900/30 p-2 rounded-lg border border-slate-800/40 italic">
                        💡 ትርጉም፦ {quizQuestions[currentQuestionIndex].amharicHint}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    {quizQuestions[currentQuestionIndex].options.map((option, index) => (
                      ...
                      >
                        ...
                        {quizSubmitted && index === quizQuestions[currentQuestionIndex].correctAnswer && <span>✓</span>}
                      </button>
                    ))}
                  </div>
                  ...
                  {!quizSubmitted ? (
                    ...
                    >
                      {currentQuestionIndex < quizQuestions.length - 1 ? "ቀጣይ ጥያቄ ▶" : "የመጨረሻ ውጤትህን እይ ▶"}
                    </button>
                  )}
                </>
              ) : (
