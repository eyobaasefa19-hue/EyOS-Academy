...
import { 
  ...
  User, Settings, AlertCircle, Check 
} from "lucide-react";
...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"courses" | "achievements" | "settings">("courses");
  ...
  // ፎቶን አነስ አድርጎ አዘጋጅ (Canvas Image Compressor)
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast("ፎቶው በስኬት ተዘጋጅቷል፤ አሁን 'አስቀምጥ' የሚለውን ይጫኑ", "success");
    } catch (err) {
      showToast("ፎቶውን ማዘጋጀት አልተቻለም፤ እባክዎ ሌላ ምስል ይሞክሩ", "error");
    }
  };
  ...
  async function handleUpdateProfile(e: React.FormEvent) {
    ...
    try {
      ...
      setIsEditOpen(false);
      showToast("ፕሮፋይልዎ በስኬት ተስተካክሏል!", "success");
      await fetchProfileData();
    } catch (err: unknown) {
      showToast(err instanceof Error ? err.message : "ስህተት ተፈጥሯል", "error");
    } finally {
      setIsUpdating(false);
    }
  }
  ...
  const handleSignOut = async () => {
    if (confirm("እርግጠኛ ነዎት መውጣት (Log Out) ይፈልጋሉ?")) {
      try {
        ...
        showToast("መውጣት አልተቻለም፤ እባክዎ እንደገና ይሞክሩ", "error");
      }
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#050b14] text-slate-200">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-semibold text-slate-400">ፕሮፋይል በመጫን ላይ...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#050b14] text-slate-200 p-4 sm:p-6 space-y-6 pb-28">
      ...
      {/* Notification Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl border backdrop-blur-md shadow-2xl transition-all ${
          toast.type === "success" 
      ...
      {/* 1. Top Navigation Bar with LOG OUT */}
      <div className="flex justify-between items-center bg-slate-900/60 border border-white/10 p-3 rounded-2xl backdrop-blur-md">
      ...
      <div className="relative rounded-3xl bg-slate-900/80 border border-white/10 p-6 backdrop-blur-xl shadow-2xl flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="flex-1 text-center sm:text-left space-y-2">
          ...
          {profile.bio && (
            <p className="text-xs text-slate-300 bg-slate-950/60 p-3 rounded-xl border border-white/5 max-w-md">
              {profile.bio}
            </p>
          )}
          ...
          <div className="space-y-1 max-w-md pt-1">
            <div className="flex justify-between text-[10px] font-bold">
              <span className="text-indigo-400">Level {profile.currentLevel || 1}</span>
              <span className="text-slate-400">{profile.levelProgressPercent || 0}% ወደ ቀጣይ Level</span>
            </div>
      ...
      {/* 3. Navigation Tabs */}
      <div className="flex bg-slate-900/60 border border-white/10 p-1 rounded-2xl gap-1">
        <button
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition cursor-pointer ${
            activeTab === "achievements" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white"
          }`}
        >
          <Award className="w-4 h-4" />
          <span>ሜዳሊያዎች & ባጆች</span>
        </button>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-900/60 p-4 rounded-2xl border border-white/10 flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-slate-400 font-bold uppercase">የተመዘገቡ ኮርሶች</p>
      ...
      {activeTab === "achievements" && (
        <div className="bg-slate-900/60 border border-white/10 rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="bg-slate-950/80 border border-indigo-500/30 p-4 rounded-2xl flex flex-col items-center text-center space-y-1">
              <ShieldCheck className="w-8 h-8 text-indigo-400 mb-1" />
              <span className="text-xs font-bold text-white">Level 1 Scholar</span>
              <span className="text-[10px] text-emerald-400 font-bold">ተከፍቷል (Unlocked)</span>
            </div>

            <div className="bg-slate-950/80 border border-amber-500/30 p-4 rounded-2xl flex flex-col items-center text-center space-y-1">
              <Trophy className="w-8 h-8 text-amber-400 mb-1" />
              <span className="text-xs font-bold text-white">Top Student</span>
              <span className="text-[10px] text-amber-400 font-bold">{profile.xpPoints} XP</span>
            </div>

            <div className="bg-slate-950/80 border border-orange-500/30 p-4 rounded-2xl flex flex-col items-center text-center space-y-1">
              <Flame className="w-8 h-8 text-orange-400 mb-1" />
              <span className="text-xs font-bold text-white">Streak Master</span>
              <span className="text-[10px] text-orange-400 font-bold">{profile.streakDays} ቀናት</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === "settings" && (
        <div className="bg-slate-900/60 border border-white/10 rounded-2xl p-5 space-y-4">
          ...
          <div className="space-y-3">
            <button
              onClick={() => setIsEditOpen(true)}
              className="w-full flex justify-between items-center p-3 rounded-xl bg-slate-950/60 border border-white/5 hover:border-indigo-500/30 transition text-xs font-bold text-slate-200 cursor-pointer"
            >
              <span>የግል መረጃዎችን ቀይር (Name, Email, Photo)</span>
              <Edit3 className="w-4 h-4 text-indigo-400" />
            </button>

            <button
              onClick={handleSignOut}
              className="w-full flex justify-between items-center p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 transition text-xs font-bold text-rose-400 cursor-pointer"
            >
              <span>ከአካውንት ውጣ (Log Out)</span>
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        </div>
